...
class TestPostProjectsNewSuccess:
    ...
    def test_agents_md_removed_after_beads_init(self):
        """After create_project, AGENTS.md generated by bd init should be deleted."""
        app, db_path, db_fd = _make_app()
        try:
            client = app.test_client()
            _auth_session(client)
            ...
            with (
                patch("app.projects.os.remove", side_effect=mock_os_remove) as mock_remove,
                patch("app.projects.os.path.exists", side_effect=mock_os_path_exists),
            ):
                self._patch_and_post(client, mocks)

            agents_md_removals = [p for p in remove_calls if "AGENTS.md" in p]
            assert len(agents_md_removals) >= 1, (
                f"Expected os.remove to be called for AGENTS.md, got calls: {remove_calls}"
            )
        finally:
            _cleanup(db_fd, db_path)
